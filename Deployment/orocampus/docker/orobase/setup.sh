#!/usr/bin/env bash
apt-get -qy update
apt-get install -qy curl locales
...
apt-get install -qqy openssh-server apt-transport-https ca-certificates vim make git-core wget curl procps \
...
php-tidy php-mysql php-curl php-gd php-mcrypt php-xmlrpc php-ldap \
...
runuser -s /bin/sh -c 'composer global require fxp/composer-asset-plugin:1.3.1' www-data || exit 1
